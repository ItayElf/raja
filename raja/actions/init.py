...
def init(directory: str = ".") -> None:
    ...
    if os.path.isdir(raja_path):
        error(f"Raja workspace already exists at {os.path.abspath(raja_path)}")
        return
    os.mkdir(raja_path)
...
def destroy(directory: str = ".") -> None:
    ...
    if not os.path.isdir(raja_path):
        error(f"Raja workspace does not exists at {os.path.abspath(directory)}")
        return
    ans = input("Delete the raja workspace? [y\\N] ")
    if ans.lower() != "y":
        return
    shutil.rmtree(raja_path)
    success(f"Raja directory at {os.path.abspath(directory)} was destroyed successfully")
